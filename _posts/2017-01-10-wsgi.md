...
WSGI (Web Server Gateway Interface) 不是服务器、python模块、框架、API或任何类型的软件，而是描述 Web 服务器如何与 Web 应用程序通信以及 Web 应用程序如何链接在一起以处理一个请求的规范。

WSGI 是一个 Python 标准，在[PEP 3333](https://www.python.org/dev/peps/pep-3333/)中有关于服务器端和应用程序接口端规范的详细描述。如果应用程序（或框架或工具包）依据WSGI规范开发，那么它将可以在任何依据WSGI规范开发的服务器上运行。

WSGI应用程序可以堆叠。 那些在堆栈中间的WSGI应用程序被称为中间件，它必须实现WSGI接口的两侧：应用程序和服务器，堆栈顶部的WSGI应用程序作为服务器，堆栈底部的WSGI应用程序作为应用程序。

![wsgi](https://raw.githubusercontent.com/yuzujin/yuzujin.github.com/master/_images/wsgi.jpg)

WSGI服务器仅从客户端接收请求，将其传递给应用程序，然后将应用程序返回的响应发送到客户端，它只做这些，其他所有的细节工作必须由应用程序或中间件完成。
